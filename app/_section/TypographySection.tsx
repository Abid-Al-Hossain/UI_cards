...
export default function TypographySection({ state, update }: Props) {
  return (
    <SectionCard title="Typography" subtitle="Text scale, weight, and font family.">
      ...
      <Slider label="Title size" value={state.titleSize} min={16} max={42} step={1} onChange={(value) => update("titleSize", value)} />
      <Slider label="Body size" value={state.bodySize} min={12} max={24} step={1} onChange={(value) => update("bodySize", value)} />
      <Slider label="Title weight" value={state.fontWeight} min={400} max={900} step={100} onChange={(value) => update("fontWeight", value)} />
    </SectionCard>
  );
}
